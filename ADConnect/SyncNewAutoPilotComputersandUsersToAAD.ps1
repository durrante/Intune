...
$computers = Get-ADComputer -Filter 'Created -ge $time' -SearchBase "OU=Autopilot Domain Join,OU=Devices,DC=alexdu,DC=co,DC=uk" -Properties Created
$users = Get-ADUser -Filter 'Created -ge $time' -SearchBase "OU=Accounts,DC=alexdu,DC=co,DC=uk" -Properties Created
